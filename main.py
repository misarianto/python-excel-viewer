...
class ExcelManager:
    def __init__(self, file_name="people.xlsx"):
...
def load_data():
    list_values = list(excel_manager.sheet.values)

    # mengambil header excel
    for col_name in list_values[0]:
        treeview.heading(col_name, text=col_name)

    # mengambil value excel dan meloopingnya
...
def insert_row():
    ...
    excel_manager.sheet.append(row_values)
    excel_manager.save()
...
name_entry.insert(0, "Name") # membuat placeholder
name_entry.bind("<FocusIn>", lambda e: name_entry.delete('0', 'end')) # hapus placeholder jika cursor aktif di inputan
